...
---
= Créez un hôte Linux NVMe over TCP E-Series dans SANtricity System Manager
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
. Cliquez sur *+Ajouter un port hôte* pour accéder au champ Ports hôtes et ajouter les identifiants de port hôte pour l'hôte. Vous pouvez ajouter un ou plusieurs identifiants de port hôte à associer à l'hôte.
...
Type de système d'exploitation hôte
 a| 
Sélectionnez l'option suivante dans la liste déroulante :
...
.Et la suite ?
Après avoir créé l'hôte, link:nvme-tcp-assign-volume-task.html["Attribuer un volume via SANtricity System Manager"].
