...
---
= Création de systèmes de fichiers dans E-Series : Linux RHEL 8, RHEL 9 et SLES 15 (NVMe over InfiniBand)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Pour RHEL 8, RHEL 9, SLES 15, vous créez un système de fichiers sur le périphérique nvme natif et montez le système de fichiers.
...
----
+
Le résultat de cette commande peut être utilisé pour trouver les périphériques associés à l' `/dev/disk/by-id/nvme-eui.[uuid#]` emplacement. Dans l'exemple ci-dessous, il s'agit de `/dev/disc/by-id/nvme-eui.000082dd5c05d39300a0980000a52225`.
