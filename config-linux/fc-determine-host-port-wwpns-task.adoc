...
summary: 'Vous installez un utilitaire FC HBA afin de pouvoir afficher le nom de port mondial (WWPN) de chaque port hôte. De plus, vous pouvez utiliser l"utilitaire HBA pour modifier les paramètres recommandés dans la colonne Notes de l"NetApp Interoperability Matrix Tool pour la configuration prise en charge.' 
---
= Déterminez les noms de ports mondiaux des hôtes Linux FC du système de stockage E-Series
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
