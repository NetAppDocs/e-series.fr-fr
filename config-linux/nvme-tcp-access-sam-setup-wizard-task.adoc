...
---
= Configurez votre système de stockage E-Series à l'aide de l'assistant de configuration SANtricity
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
SANtricity System Manager est une interface web intégrée à chaque contrôleur. Pour accéder à l'interface utilisateur, vous devez saisir l'adresse IP du contrôleur dans un navigateur. Un assistant de configuration vous aide à démarrer la configuration du système.
...
.Et la suite ?
Après avoir configuré le stockage dans SANtricity System Manager, link:nvme-tcp-configure-switch-task.html["configurer les commutateurs"].
