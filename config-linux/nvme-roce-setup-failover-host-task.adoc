...
SLES 12 utilise Device Mapper Multipath (DMMP) pour les chemins d’accès multiples pour NVMe over RoCE. RHEL 8, RHEL 9 et SLES 15 utilisent un basculement NVMe natif intégré. Selon le système d’exploitation que vous exécutez, une configuration supplémentaire des chemins d’accès multiples est nécessaire pour le faire fonctionner correctement.
