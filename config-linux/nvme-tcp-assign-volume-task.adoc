...
summary: 'Vous devez affecter un volume (namespace) à un hôte ou à un cluster hôte afin de pouvoir utiliser les opérations d"E/S. Cette affectation permet à un hôte ou un cluster hôte d"accéder à un ou plusieurs namespaces d"une baie de stockage.' 
---
= Attribuer un volume à un hôte Linux NVMe over TCP dans le système de stockage E-Series
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
.Étapes
. Dans le panneau de gauche de l'interface SANtricity System Manager, sélectionnez menu:Storage[Hosts].
...
.Et la suite ?
Après avoir attribué un volume, link:nvme-tcp-display-volumes-visible-host-task.html["afficher les volumes visibles à l'hôte"].
