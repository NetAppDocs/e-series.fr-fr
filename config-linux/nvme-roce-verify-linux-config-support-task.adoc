...
---
= Vérifiez la prise en charge de la configuration Linux pour NVMe over RoCE dans le système de stockage E-Series
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
CAUTION: *RISQUE DE PERTE DE DONNÉES.* Vous devez exécuter NVMe sur RoCE dans un environnement sans perte. Activez le contrôle de flux de bout en bout à l'aide des trames de pause globales ou du contrôle de flux prioritaire (PFC) sur votre réseau afin d'éliminer le risque de perte de données. Le PFC exige que NVMe sur RoCE soit étiqueté comme trafic de priorité 3 avec une valeur DSCP de `106`.
...
.Et la suite ?
Après avoir vérifié la prise en charge de la configuration Linux et examiné les restrictions, link:nvme-roce-configure-ip-address-dhcp-task.html["Configurer les adresses IP via DHCP"].
