...
---
= Afficher les volumes visibles par un hôte Linux via NVMe over RoCE dans le système de stockage E-Series
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Pour afficher des informations sur chaque chemin NVMe vers un volume E-Series, utilisez la `nvme netapp smdevices [-o <format>]` commande. La sortie <format> peut être normale (par défaut si -o n'est pas utilisé), colonne ou json.
...
----
.Et la suite ?
Après avoir visualisé les volumes visibles, link:nvme-roce-setup-failover-host-task.html["Configurer le basculement sur l'hôte"].
