...
summary: 'Utilisez l"outil SMDevices, qui fait partie du package nvme-cli, pour afficher les volumes actuellement visibles sur l"hôte. Il s"agit d"une alternative à la commande nvme list.' 
---
= Affichage des volumes visibles pour l'hôte dans les E-Series - Linux (NVMe over InfiniBand)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Pour afficher des informations sur chaque chemin NVMe vers un volume E-Series, utilisez la `nvme netapp smdevices [-o <format>]` commande. La sortie `<format>` peut être normal (par défaut si -o n'est pas utilisé), colonne ou json.
