...
---
= Configurer la mise en réseau iSCSI côté baie pour les hôtes Linux dans le système de stockage E-Series
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
Cette procédure explique comment accéder à la configuration des ports iSCSI depuis la page Matériel du System Manager. Vous pouvez également accéder à la configuration depuis menu:System[Paramètres > Système > Configurer les ports iSCSI].
