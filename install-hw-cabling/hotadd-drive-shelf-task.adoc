...
---
= Ajoutez à chaud une étagère de disques dans un système de stockage E-Series
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Vous pouvez ajouter un nouveau châssis de disques tout en maintenant l'alimentation des autres composants du système de stockage, à condition que les contrôleurs de votre système de stockage disposent de ports SAS disponibles. Vous pouvez configurer, reconfigurer, ajouter ou déplacer la capacité du système de stockage sans interrompre l'accès des utilisateurs aux données.
