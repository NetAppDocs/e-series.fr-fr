...
---
= Configurer les connexions iSCSI pour les hôtes Windows dans le système de stockage E-Series
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
* Deux réseaux distincts pour une haute disponibilité. Assurez-vous d'isoler votre trafic iSCSI sur des segments de réseau séparés des autres types de trafic en utilisant des VLAN ou deux réseaux distincts.
...
Cette tâche explique comment accéder à la configuration du port iSCSI depuis la page Matériel. Vous pouvez également accéder à la configuration depuis menu : Paramètres [Système > Configurer les ports iSCSI].
...
. Dans le panneau de gauche, sélectionnez menu:Hardware[Controllers and components].
