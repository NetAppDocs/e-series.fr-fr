...
---
= Remplacez un HIC défectueux dans un système de stockage EF300 ou EF600
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
Lors du remplacement d'un HIC défaillant, vous devez mettre hors ligne un contrôleur pour placer le système dans un état non redondant, remplacer le HIC, puis remettre le contrôleur en ligne.


NOTE: Il n'est pas nécessaire de mettre hors tension la baie de stockage EF300 et EF600 lors du remplacement d'une HIC du même type de protocole (par exemple, le remplacement d'une HIC Fibre Channel par une autre HIC Fibre Channel).

.Avant de commencer
* Révision link:hics-overview-supertask-concept.html["Conditions requises pour les mises à niveau des HIC EF300 ou EF600"].
* Planifiez une fenêtre de maintenance avec interruption pour cette procédure. Vous ne pouvez pas accéder aux données de la baie de stockage tant que vous n'avez pas terminé cette procédure avec succès. Comme les deux contrôleurs doivent avoir la même configuration HIC lorsqu'ils sont sous tension, vous devez mettre un contrôleur hors ligne pour placer le système dans un état non redondant lors de l'installation des HIC.
...
+
.. Sélectionnez le menu : Stockage [Volumes]. Sur la page *Tous les volumes*, vérifiez que les volumes sont bien attribués à leurs propriétaires respectifs. Sélectionnez l’icône *ellipse* > *Changer de propriétaire* pour afficher les propriétaires des volumes.
