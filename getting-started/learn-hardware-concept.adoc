...
Vous trouverez ci-dessous quelques exemples de modèles de baies E-Series disponibles. Pour obtenir la liste complète des modèles de baies E-Series et leurs spécifications matériel, consultez le NetApp Hardware Universe à  https://hwu.netapp.com["NetApp Hardware Universe"^].

* Série EF50 -- modèle d'entrée de gamme tout flash, tout NVMe
* Série EF80 -- milieu de gamme, tout flash, tout NVMe
...
* Série EF300C -- tout flash, NVMe haute densité d'entrée de gamme
* Série EF600C -- milieu de gamme tout flash, NVMe haute densité
* Série E2800 (Fin de disponibilité) – hybride d'entrée de gamme
* Série EF280 (Fin de disponibilité) – 100 % flash d'entrée de gamme
* Série E5700 (Fin de disponibilité) – hybride de milieu de gamme
* Série EF570 (Fin de disponibilité) -- milieu de gamme tout flash



NOTE: Pour SANtricity OS 11.80GA ou version ultérieure, tous les ports USB sont désactivés pendant l'exécution de SANtricity OS.
...
+
--
* Prend en charge les disques SSD NVMe avec les capacités suivantes :
+
** 3,8 To
** 7,6 To
** 15,3 To
...
NOTE: Pour les disques de 30 To et 60 To, l'EF50 ne prend en charge que l'utilisation de pools de disques dynamiques, sans prise en charge RAID héritée.



* Prend en charge jusqu'à 24 disques SSD NVMe dans le compartiment du contrôleur.
* Prise en charge d'une ou deux cartes d'interface d'hôte par contrôleur.
* Prise en charge des cartes et câbles inter-contrôleurs pour la communication double contrôleur.
+

NOTE: Il est essentiel de connecter correctement les câbles de duplication lors de la configuration de votre système EF50. Pour plus d'informations, voir  https://docs.netapp.com/us-en/e-series/install-hw-ef50-ef80/install-cable.html#step-1-cable-the-inter-controller-mirroring-connections["Câblez les connexions de mise en miroir entre contrôleurs - EF50 et EF80"].

* Prise en charge d'une carte d'extension SAS en option, permettant la connexion des tiroirs d'extension SAS.
* Un pool de disques unique est créé automatiquement s'il y a suffisamment de disques non attribués lors du démarrage du système.
* À partir de SANtricity OS 12.10, le EF50 prend en charge la combinaison de disques de 3,8 To, 7,6 To et 15,3 To avec des disques de 30 To et 60 To. Ces disques dans une baie mixte seront automatiquement placés dans un seul pool de disques sans prise en charge RAID.
+

NOTE: Les pools de disques créés automatiquement peuvent être supprimés via SANtricity System Manager ou SANtricity CLI. Pour plus d’informations, consultez  https://docs.netapp.com/us-en/e-series-cli/commands-a-z/delete-diskpool.html["Supprimer un pool de disques - SANtricity CLI Guide"^] et  https://docs.netapp.com/us-en/e-series-santricity/sm-storage/delete-pool-or-volume-group.html["Supprimer un pool ou un groupe de volumes - Guide de l'administrateur système SANtricity"^].
...
NOTE: Pour les disques de 30 To et 60 To, l'EF80 ne prend en charge que l'utilisation de pools de disques dynamiques, sans prise en charge RAID héritée.



* Prend en charge jusqu'à 24 disques SSD NVMe dans le compartiment du contrôleur.
* Prise en charge d'une, deux ou trois cartes d'interface d'hôte par contrôleur.
* Prise en charge des cartes et câbles inter-contrôleurs pour la communication double contrôleur.
+

NOTE: Il est crucial de connecter correctement les câbles de duplication lors de la configuration de votre système EF80. Pour plus d'informations, voir https://docs.netapp.com/us-en/e-series/install-hw-ef50-ef80/install-cable.html#step-1-cable-the-inter-controller-mirroring-connections["Câblez les connexions de mise en miroir entre contrôleurs - EF50 et EF80"].

* Prise en charge d'une carte d'extension SAS en option, permettant la connexion des tiroirs d'extension SAS.
+
Si la carte d'extension SAS est installée, le nombre maximal de cartes d'interface d'hôte pour la connectivité hôte est limité à deux par contrôleur.

* Un pool de disques unique est créé automatiquement s'il y a suffisamment de disques non attribués lors du démarrage du système.
* À partir de SANtricity OS 12.10, le EF80 prend en charge la combinaison de disques de 3,8 To, 7,6 To et 15,3 To avec des disques de 30 To et 60 To. Ces disques dans une baie mixte seront automatiquement placés dans un seul pool de disques sans prise en charge RAID. + REMARQUE : Les pools de disques créés automatiquement peuvent être supprimés via SANtricity System Manager ou SANtricity CLI. Pour plus d’informations, consultez  https://docs.netapp.com/us-en/e-series-cli/commands-a-z/delete-diskpool.html["Supprimer un pool de disques - SANtricity CLI Guide"^] et  https://docs.netapp.com/us-en/e-series-santricity/sm-storage/delete-pool-or-volume-group.html["Supprimer un pool ou un groupe de volumes - Guide de l'administrateur système SANtricity"^].
