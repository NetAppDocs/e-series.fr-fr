...
summary:  
---
= Remplacer une carte d'interface d'hôte E4000
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Depuis la page d'accueil de SANtricity System Manager, sélectionnez menu : Storage [Operations in Progress].
...
+
.. Sélectionnez *Stockage › Volumes*. Depuis la page *Tous les volumes*, vérifiez que les volumes sont répartis entre leurs propriétaires préférés. Sélectionnez *icône en forme de points de suspension › Modifier le propriétaire* pour afficher les propriétaires des volumes.
