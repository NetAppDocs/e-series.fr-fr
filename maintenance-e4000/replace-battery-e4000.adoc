...
summary:  
---
= Remplacez la batterie d'un système de stockage E4000
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
... Sélectionnez le menu : Hardware [Controllers and components] pour afficher les contrôleurs.
...
** Depuis SANtricity System Manager :
+
... Sélectionnez le menu : Matériel [Controller & Components].
...
+
.. Sélectionnez *Stockage › Volumes*. Depuis la page *Tous les volumes*, vérifiez que les volumes sont répartis entre leurs propriétaires préférés. Sélectionnez *icône en forme de points de suspension › Modifier la propriété des volumes* pour afficher les propriétaires des volumes.
.. Si tous les volumes appartiennent au propriétaire privilégié, continuez à l’étape 4.
.. Si aucun des volumes n'est restitué, vous devez restituer les volumes manuellement. Accédez à *ellipse icon › Redistribuer les volumes*.
